...
def _payload(phone="0991112233", first="Ada", last="Lovelace"):
    return {"phone": phone, "firstName": first, "lastName": last}
...
def test_list_and_search_clients(client):
    client.post("/api/v1/clients/", json=_payload(phone="0990000001", first="Grace"))
    client.post("/api/v1/clients/", json=_payload(phone="0990000002", first="Alan"))
...
def test_get_client_by_phone(client):
    client.post("/api/v1/clients/", json=_payload(phone="0995554433"))
    ok = client.get("/api/v1/clients/phone/0995554433")
    assert ok.status_code == 200
    assert ok.json()["phone"] == "0995554433"

    missing = client.get("/api/v1/clients/phone/0000000000")
